...
class translate extends application {

    const application_name = 'Translate';
    const author = 'aramok';
    const version = '1.0';
    const width = 540;
    const height = 380;
    ...
    function __construct($app_file_name, $window_id, $file) {
        ...
        $content .= '<script>
                function calculate_hash' . $window_id . '(event){
                    ...
                    var dataObject = {};
                    dataObject["amount"] = $("#' . $window_id . 'amount").val();
                    dataObject["currency"]   = $("#' . $window_id . 'currency").val();
                    dataObject["convertto"]   = $("#' . $window_id . 'convertto").val();
                        $.ajax({
                            url: "' . $imgdir . '/convert.php",
                            type: "post" ,
                            data: dataObject,
                            success: function (data, status) {
                                $("#' . $window_id . 'result").val(data);
                            }}); // end ajax call
                }
                function reverse_currencies' . $window_id . '(event){
                    event.preventDefault();
                    var temp = $("#' . $window_id . 'currency").val();
                    $("#' . $window_id . 'currency").val($("#' . $window_id . 'convertto").val());
                    $("#' . $window_id . 'convertto").val(temp);
                }
                </script>';
        ...
        <div class="row">
            ...
            <div class="cell systeminfo-cell">
                Amount:<br>
                <input type="text" class="input" size="40" id="' . $window_id . 'amount"><br><br>
                Currency:<br>
                <select class="select" id="' . $window_id . 'currency">
...
<option  value="ZWL">Zimbabwean Dollar (2009) (ZWL)</option>
                ...
                <a class="button" href="#" onclick="reverse_currencies' . $window_id . '(event)"> Reverse </a>
                <br><br>
                Convert to:<br>
                <select class="select" id="' . $window_id . 'convertto">
                     <option  value="AED">United Arab Emirates Dirham (AED)</option>
<option  value="AFN">Afghan Afghani (AFN)</option>
<option  value="ALL">Albanian Lek (ALL)</option>
<option  value="AMD">Armenian Dram (AMD)</option>
...
<option  value="ZWL">Zimbabwean Dollar (2009) (ZWL)</option>
                ...
                Result:<br>
                <input type="text" class="input" size="55" id="' . $window_id . 'result"><br>
                <span class="alright">
